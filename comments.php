...
*/
	...
	{	
		...
		<div align="center">
			<p style="font-size:22px; text-decoration:none">
				<a style="text-decoration:none" href="comments.php"><button class="turquoise-flat-button" style="background:#FC4144">Go Back</button></a> <!-- Button for returning to the home page -->
			</p>
		</div>
		<?php
	}
	...
	else
	{
		...
		$post_number = $_GET["post"]; //Current ID of the post
		$query = "SELECT * FROM posts WHERE id='$post_number' AND deleted=0";
		$info2 = mysql_fetch_object(mysql_query($query));
	
		$time = strtotime($info2->date); //Date the post was submitted
		$submitted = date("m/d/y \a\\t g:i A", $time); //Date formatted properly to 'Month/Day/Year at Minute:Second A/PM'
	
		$rep = "SELECT * FROM replies WHERE post='$post_number' AND deleted=0"; //Select all the replies to this post using the ID as the indentifier
		$replies = mysql_query($rep) or trigger_error(mysql_error())." ".$rep;
		$count = mysql_num_rows($replies); //Count the number of replies
