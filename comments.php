...
*/
	...
	{        
		...
		echo '<td colspan="4" style="padding-left: 10px;">
				...
				<p style="font-size:12px;padding:0;text-align:left;display:inline">
					<a id="like_'.$post_number.'" style="text-decoration:none;color:#1F80C9;display:inline" href="#" onclick="like_add('.$post_number.');return false;">'.$button.'</a> 
				</p>
		...
		while($replies2 = mysql_fetch_object($replies)) //QUERY for all of the comments on this post
		{
			...
			$reply = stripslashes(submit::auto_link_text($replies2->reply)); //Check the reply for links
  			...
  			echo '<td colspan="4" style="position:relative"> 
					...
					<p style="font-size:14px;color:000;margin:0;padding-left:40px;">
